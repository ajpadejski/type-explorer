...
function FontInput({ label, onLoad }) {
  ...
  const pill = (id, txt) => (
    <button key={id} onClick={() => { setMode(id); setStatus({ text: "", ok: true }); }} style={{
      ...
      {mode === "css" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <span style={{ fontSize: 10, color: "var(--color-text-tertiary)" }}>Note: third-party CDNs may be blocked in this sandbox. File upload always works.</span>
          </div>
        </div>
      )}
